...
//      );
//      if (this._codeVerified) {
//        await _finishSignIn(_firebaseUser, context);
//      } else {
//        progressDialog.hide();
//        _showErrorSnackbar("Invalid Code");
//      }
//    });
  ...
  void updateFirebaseToken(BuildContext context) {
    String _token;
    DatabaseReference ref11 = FirebaseDatabase.instance.reference().child("users_tokens");
    _firebaseMessaging.getToken().then((token) {
      _token = token;
      var data =
      {"token": _token};
      FirebaseAuth.instance.currentUser().then((user) {
        ref11.child(user.uid).set(data).whenComplete(() {
  ...
  void sginInAnonymously() {
    FirebaseAuth.instance.signInAnonymously().then((firebase) {
      ...
      ref.child(firebase.user.uid).once().then((DataSnapshot datasnapshot) {
  ...
  signIn_User(BuildContext context, FirebaseUser userr) {
    progressDialog.hide();
    ref.child(userr.uid).once().then((DataSnapshot datasnapshot) {
      if (datasnapshot == null || datasnapshot.value == null) {
        user_defualts
                          ...
                          children: <Widget>[
                            Expanded(
                                child: Container(
                                  child: Column(
                                    mainAxisAlignment: MainAxisAlignment.start,
                                    children: <Widget>[_buildResendSmsWidget(context)],
                                  ),
                                ))
                          ],
                        ),
