...
import 'package:flutter_messaging_app/app/team_list_fragment_tab.dart';
...
class AppointmentListFragment extends StatefulWidget {
  @override
  _AppointmentListFragmentState createState() =>
      _AppointmentListFragmentState();
}
...
class _AppointmentListFragmentState extends State<AppointmentListFragment> {
  ...
  DatabaseReference pen_challenge_db = FirebaseDatabase.instance
      .reference()
      .child("challenge_requests/pending_challenges");
  DatabaseReference ground_match_requests = FirebaseDatabase.instance
      .reference()
      .child("ground_match_requests/pending_requests");
  DatabaseReference ground_match_reject_requests = FirebaseDatabase.instance
      .reference()
      .child("ground_match_requests/rejected_requests");
  DatabaseReference match_apprequest_db = FirebaseDatabase.instance
      .reference()
      .child("ground_match_requests/approved_requests");
  DatabaseReference app_request_db = FirebaseDatabase.instance
      .reference()
      .child("join_team_request/approved_requests");
  DatabaseReference reject_request_db = FirebaseDatabase.instance
      .reference()
      .child("challenge_requests/rejected_challenges");
  DatabaseReference accep_request_db = FirebaseDatabase.instance
      .reference()
      .child("challenge_requests/accepted_challenges");
      ...
      ground_match_requests
        ...
        if (datasnapshot.value != null) {
          ...
          for (var key in keys) {
            ...
            teams_db
                .child(sender_team_id)
                .once()
                .then((DataSnapshot teamsnapshot) {
                ...
                teams_db
                  ...
                  if (teamsnapshot.value != null) {
                    ...
                    grounds_db
                        .child(ground_id)
                        .once()
                        .then((DataSnapshot groundSnapshot) {
                            ...
                            child: pendingRequests.length == 0
                                ? Column(
                                    children: <Widget>[
                                      RaisedButton(
                                        child: Text(
                                            'Buraya bas ve studio bilgilerini gir'),
                                        onPressed: () =>
                                            _studioDetailSignInPage(context),
                                      ),
                                      RaisedButton(
                                        child: Text('Takım listesini gör'),
                                        onPressed: () => _teamList(context),
                                      ),
                                      new Text(
  ...
  void _studioDetailSignInPage(BuildContext context) {
    Navigator.push(context,
        MaterialPageRoute<String>(builder: (BuildContext context) {
      return StudioDetailSignInPage();
    }));
  }

  void _teamList(BuildContext context) {
    Navigator.push(context,
        MaterialPageRoute<String>(builder: (BuildContext context) {
  ...
  void rejectJoiningRequest(
    ...
    users_tokens_db
        .child(pendingRequest.sender_team_id)
        .once()
        .then((DataSnapshot datasnapshot) {
      if (datasnapshot.value != null) {
        setState(() {
    ...
    users_tokens_db
        .child(pendingRequest.opponent_team_id)
        .once()
        .then((DataSnapshot datasnapshot) {
      if (datasnapshot.value != null) {
        setState(() {
    ...
    ground_match_requests
        .child(pendingRequest.ground_id)
        .child(pendingRequest.sender_team_id)
        .remove();
      ...
      grounds_db
        ...
        if (datasnap.value != null) {
          setState(() {
            ...
            String body =
                "${ground_owner}'s owner rejected your Match Request.\n Reason: ${reason}";

            if (sender_token != "") {
              callOnFcmApiSendPushNotifications(sender_token, title, body);
            } else {
              sendNotificationDataToFirebase(
                  pendingRequest.sender_team_id, title, body);
            }
            if (opponent_token != "") {
              callOnFcmApiSendPushNotifications(opponent_token, title, body);
            } else {
              sendNotificationDataToFirebase(
                  pendingRequest.opponent_team_id, title, body);
            }
          });
        }
      });
        ...
        if (datasnap.value != null) {
          setState(() {
            ...
            grounds_db
                .child(pendingRequest.ground_id)
                .child("Slots")
                .child(pendingRequest.date)
                .child(pendingRequest.slot_id)
                .update(data1)
                .whenComplete(() {
              String title1 = "Schedule Match";
              String body1 =
                  "Match between ${pendingRequest.opponent_team_name} and ${pendingRequest.sender_team_name} on \n ${pendingRequest.date} - ${pendingRequest.slot_time}  at ${pendingRequest.ground_name}";
