...
class TeamProfileAboutFragment extends StatefulWidget {

  @override
  _TeamProfileAboutFragmentState createState() => _TeamProfileAboutFragmentState();
}

class _TeamProfileAboutFragmentState extends State<TeamProfileAboutFragment> {
  ...
  void getData() async {
     id = await user_defualts.getTeamID();
    setState(() {
      print("User _id: $id");
      _getUserData();
    });
  ...
  void _getUserData()
  {
      ...
      _fullName="";
      ref.child(id).once().then((DataSnapshot datasnapshot) {
        Map<dynamic,dynamic> user =datasnapshot.value;
        if (datasnapshot != null) {
          setState(() {
            _fullName = user['team_name'];
            _location = user['city'];
            drawer_picture = user['team_logo'];
            _age = user['about'].toString();
          });
        }
      });

      ref.child(id).child("Players").once().then((DataSnapshot datasnapshot) {
        Map<dynamic,dynamic> user =datasnapshot.value;
    ...
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          _buildStatItem("Total Players", total_players.toString()),
        ],
      ),
    );
  }
