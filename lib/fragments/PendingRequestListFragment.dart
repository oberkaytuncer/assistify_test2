...
class PendingRequestListFragment extends StatefulWidget {
  @override
  _PendingRequestListFragmentState createState() =>
      _PendingRequestListFragmentState();
}

class _PendingRequestListFragmentState
    extends State<PendingRequestListFragment> {
                            ...
                            child: new Text(
                              ...
                              new Container(
                               margin: const EdgeInsets.only(right: 10.0),
                                alignment: FractionalOffset.topLeft,
                                child: new Text(
                                    "${detail.player_name} wants to Join you Team(${detail.team_name}).",
                                  style: TextStyle(
...
//                                      ))),
  ...
  void rejectJoiningRequest(PendingRequest pendingRequest, BuildContext context,int index) {
    ...
    progressDialog.show();
    String token="";
    ref_token.child(pendingRequest.player_id).once().then((DataSnapshot datasnapshot){
    ...
    String body = "${pendingRequest.team_name}'s captain rejected your Request.";
    join_team_db.child(pendingRequest.team_id).child(pendingRequest.player_id).remove();
    var data={
    ...
    join_team_db2.child(pendingRequest.team_id).child(pendingRequest.player_id).set(data).whenComplete(()
        {
          ...
          if(token!="") {
            ...
            {
              sendNotificationDataToFirebase(pendingRequest.player_id,title,body);
            }
          setState(() {
  ...
  void acceptJoiningRequest(PendingRequest pendingRequest, BuildContext context,int index) {
    ...
    String body = "${pendingRequest.team_name}'s captain accepted your Request.";
    join_team_db.child(pendingRequest.team_id).child(pendingRequest.player_id).remove();
    var data={
      "player_id":pendingRequest.player_id,
      "team_id": pendingRequest.team_id,
